...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (currentState == AppState::DIMENSION_SELECT) {
            ...
            if (IsKeyPressed(KEY_DOWN)) sandboxSelection = (sandboxSelection + 1) % 5; // Updated to 5
            if (IsKeyPressed(KEY_UP)) sandboxSelection = (sandboxSelection - 1 + 5) % 5;

            int slideSpeed = IsKeyDown(KEY_LEFT_SHIFT) ? 50 : 10;

            if (sandboxSelection == 0 && (IsKeyPressed(KEY_LEFT) || IsKeyPressed(KEY_RIGHT))) {
                int current = (int)mySettings.init_state_2d;
                mySettings.init_state_2d = static_cast<InitialState2D>((current + 1) % 3);
            }
            if (sandboxSelection == 1) {
                if (IsKeyPressed(KEY_RIGHT)) mySettings.resolution += slideSpeed;
                if (IsKeyPressed(KEY_LEFT)) mySettings.resolution -= slideSpeed;
            ...
            // NEW: Boundary Toggle
            if (sandboxSelection == 3 && (IsKeyPressed(KEY_LEFT) || IsKeyPressed(KEY_RIGHT))) {
            ...
            if (IsKeyPressed(KEY_ENTER) && sandboxSelection == 4) { // Launch moved to 4
                mySettings.mode = SimMode::QUANTUM;
                mySettings.num_barriers = 0; 
                mySettings.target_position = -1;
            ...
            // 6 options total
            if (IsKeyPressed(KEY_DOWN)) virusConfigSelection = (virusConfigSelection + 1) % 6;
            if (IsKeyPressed(KEY_UP)) virusConfigSelection = (virusConfigSelection - 1 + 6) % 6;
            if (IsKeyPressed(KEY_D)) currentState = AppState::MAIN_MENU;
            ...
            // ... (Decoherence and Boundary logic placeholders go here) ...

            // 4. NEW: Grid Resolution Slider
            int slideSpeed = IsKeyDown(KEY_LEFT_SHIFT) ? 50 : 10;
            if (virusConfigSelection == 4) {
                if (IsKeyPressed(KEY_RIGHT)) mySettings.resolution += slideSpeed;
                if (IsKeyPressed(KEY_LEFT)) mySettings.resolution -= slideSpeed;
                if (mySettings.resolution < 10) mySettings.resolution = 10; // Prevent array crashes
            }

            // Launch Simulator
            if (IsKeyPressed(KEY_ENTER) && virusConfigSelection == 5) {
                mySettings.mode = SimMode::QUANTUM;
                ...
                std::string target_file = "data/test_mini_jhu.csv"; 
                std::vector<ViralHotspot> raw_data = ParseDiseaseData(target_file, timeStepMode);
        ...
        if (currentState == AppState::DIMENSION_SELECT) {
            ...
            DrawText("INITIAL SPATIAL STATE:", col1, 180, 20, (sandboxSelection == 0) ? YELLOW : GRAY);
            ...
            else stateText = "< ALTERNATING PHASE >";
            DrawText(stateText, col2, 180, 20, (sandboxSelection == 0) ? WHITE : LIGHTGRAY);

            DrawText("GRID RESOLUTION:", col1, 230, 20, (sandboxSelection == 1) ? YELLOW : GRAY);
            DrawText(TextFormat("< %dx%d Nodes >", mySettings.resolution, mySettings.resolution), col2, 230, 20, (sandboxSelection == 1) ? WHITE : LIGHTGRAY);

            DrawText("SIMULATION SPEED:", col1, 280, 20, (sandboxSelection == 2) ? YELLOW : GRAY);
            DrawText(TextFormat("< %d Steps / Frame >", mySettings.steps_per_frame), col2, 280, 20, (sandboxSelection == 2) ? WHITE : LIGHTGRAY);

            DrawText("BOUNDARY CONDITION:", col1, 330, 20, (sandboxSelection == 3) ? YELLOW : GRAY);
            const char* boundsText = (mySettings.boundary_condition == BoundaryType::ABSORBING) ? "< ABSORBING (Void) >" : "< REFLECTIVE (Walls) >";
            DrawText(boundsText, col2, 330, 20, (sandboxSelection == 3) ? WHITE : LIGHTGRAY);

            DrawText("[ LAUNCH SANDBOX ]", screenWidth/2 - MeasureText("[ LAUNCH SANDBOX ]", 20)/2, 450, 20, (sandboxSelection == 4) ? GREEN : GRAY);
            DrawText("Hold SHIFT to adjust resolution faster", screenWidth/2 - MeasureText("Hold SHIFT to adjust resolution faster", 15)/2, 550, 15, LIGHTGRAY);
        }
            ...
            DrawRectangle(0, 0, screenWidth, screenHeight, { 30, 40, 50, 255 });
            DrawText("CONFIGURE DATA INGESTION", screenWidth/2 - MeasureText("CONFIGURE DATA INGESTION", 30)/2, 80, 30, WHITE);
            DrawText(TextFormat("Dataset: %s", selectedDataset), screenWidth/2 - MeasureText(TextFormat("Dataset: %s", selectedDataset), 20)/2, 120, 20, LIGHTGRAY);
            
            // Widened the gap between the columns slightly
            int col1 = screenWidth/2 - 260; 
            int col2 = screenWidth/2 + 30;

            DrawText("TIME STEP AGGREGATION:", col1, 200, 20, (virusConfigSelection == 0) ? YELLOW : GRAY);
            const char* timeText = (timeStepMode == 0) ? "< DAILY >" : (timeStepMode == 1) ? "< WEEKLY >" : "< MONTHLY >";
            DrawText(timeText, col2, 200, 20, (virusConfigSelection == 0) ? WHITE : LIGHTGRAY);

            // Cleaned up the duplicates and overlapping Map Mode text
            DrawText("GEOGRAPHIC BOUNDS:", col1, 250, 20, (virusConfigSelection == 1) ? YELLOW : GRAY);
            const char* boundsText = (mapPresetMode == 0) ? "< GLOBAL >" : (mapPresetMode == 1) ? "< UNITED STATES >" : "< EUROPE >";
            DrawText(boundsText, col2, 250, 20, (virusConfigSelection == 1) ? WHITE : LIGHTGRAY);

            DrawText("DECOHERENCE (p):", col1, 300, 20, (virusConfigSelection == 2) ? YELLOW : GRAY);
            DrawText("< 0.05 (Quantum Hybrid) >", col2, 300, 20, (virusConfigSelection == 2) ? WHITE : LIGHTGRAY);

            DrawText("BOUNDARY CONDITION:", col1, 350, 20, (virusConfigSelection == 3) ? YELLOW : GRAY);
            DrawText("< STICKY (Accumulation) >", col2, 350, 20, (virusConfigSelection == 3) ? WHITE : LIGHTGRAY);

            DrawText("GRID RESOLUTION:", col1, 400, 20, (virusConfigSelection == 4) ? YELLOW : GRAY);
            DrawText(TextFormat("< %dx%d Nodes >", mySettings.resolution, mySettings.resolution), col2, 400, 20, (virusConfigSelection == 4) ? WHITE : LIGHTGRAY);

            DrawText("[ BOOT SIMULATOR ]", screenWidth/2 - MeasureText("[ BOOT SIMULATOR ]", 20)/2, 480, 20, (virusConfigSelection == 5) ? GREEN : GRAY);
            DrawText("Hold SHIFT to adjust resolution faster", screenWidth/2 - MeasureText("Hold SHIFT to adjust resolution faster", 15)/2, 550, 15, LIGHTGRAY);
        }
